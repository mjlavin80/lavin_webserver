...
class Main(db.Model):
    ...
    audio_sample_url = db.Column(db.String(512))   
...
class FullReview(db.Model):
    __tablename__="full_review"
    _id = db.Column(db.Integer, primary_key=True)
    main_id = db.Column(db.Integer(), db.ForeignKey('main._id', ondelete='CASCADE'))
    date_scraped = db.Column(db.DateTime, index=True) 
...
class PriceReviews(db.Model):
    ...
    date_scraped_price=db.Column(db.Float, index=True)
