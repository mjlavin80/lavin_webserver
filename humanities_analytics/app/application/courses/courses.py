...
def load_course(semester=None, course=None, template=None):
    ...
    cn = course.replace("-", " ").capitalize().replace("Data ", "DATA ").replace("Da ", "DA ").replace('Fy sem', 'FYSEM')
...
@courses_blueprint.route("/da-283")
@courses_blueprint.route("/da-283/")
@courses_blueprint.route("/da-283/<semester>")
@courses_blueprint.route("/da-283/<semester>/")
def da_283(semester=None, course="da-283", template="syllabus.html"):
    return load_course(semester=semester, course=course, template=template)
