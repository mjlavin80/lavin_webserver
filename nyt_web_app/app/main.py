...
def index(nyt_id=None):
    if current_user.is_authenticated and current_user.is_admin:
        if nyt_id != None:
            ...
            row = Metadata().query.filter(and_(Metadata.year > 1905, Metadata.year < 1925)).filter(Metadata.review_type.like("needs_audit%")).filter(Metadata.page.in_('BR1', 'BR2', 'BR3', 'BR4', 'BR5', 'BR6', 'BR7', 'BR8', 'BR9', 'BR10')).order_by(func.rand()).first()
            endpoint = row.nyt_pdf_endpoint
