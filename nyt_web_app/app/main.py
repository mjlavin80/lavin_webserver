...
def index(nyt_id=None):
    if current_user.is_authenticated and current_user.is_admin:
        if nyt_id != None:
            ...
            row = Metadata().query.filter(and_(Metadata.review_type == "needs_audit", Metadata.year > 1905, Metadata.year < 1926)).order_by(func.rand()).first()
            endpoint = row.nyt_pdf_endpoint
...
def female(nyt_id=None):
    if current_user.is_authenticated and current_user.is_admin:
        if nyt_id != None:
            ...
            row = Metadata().query.filter(and_(Metadata.review_type == "needs_audit_probably_female", Metadata.year > 1905, Metadata.year < 1926)).order_by(func.rand()).first()
            endpoint = row.nyt_pdf_endpoint
            return render_template("index.html", nyt_id=row.nyt_id, row=row, endpoint=endpoint) 
    else:
        return render_template("index.html", nyt_id=None, endpoint=None)
